...
if ($remarks === "") {
    echo json_encode(["status"=>false,"message"=>"Remarks are required for rejection."]);
    exit;
}
...
if (!$updateStmt->execute() || $updateStmt->affected_rows <= 0) {
    echo json_encode(["status"=>false,"message"=>"Unable to reject leave or leave was already processed."]);
...
$message = "Your leave request from " .
           ...
           " has been rejected by the HOD. Reason: " . $remarks;

createLeaveNotification($conn, $leave["reg_no"], $leaveId, $title, $message, "hod_rejected");

echo json_encode(["status"=>true,"message"=>"Leave rejected by HOD and student notified."]);
